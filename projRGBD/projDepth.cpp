...
//    const std::string finCalib="/home/da/data/thermal2Kinect.yml";
    ...
    LoadSingleFile(finNames, hostNames);
    ...
    if(targetPath.at( targetPath.length() - 1 ) != '/') targetPath = targetPath+"/";
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                ...
                const Eigen::Vector3d pixelTarget = targetK * pcTarget;
                 ...
                 std::cout << "i " << i << " j " << j << "\n";
                 std::cout << "depth kinect " << depth << " depth rgb " << depthTarget << std::endl;
                 std::cout << "pixel kinect " << pixel.transpose() << std::endl;
                 std::cout << "pc kinect " << pcHost.transpose() << std::endl;
                 std::cout << "pixel Target " << pixelTarget.transpose() << std::endl;
                 std::cout << "pc target " << (pcTarget * depthTarget).transpose() << std::endl;
                 std::cout << "\n\n\n\n\n\n ";
